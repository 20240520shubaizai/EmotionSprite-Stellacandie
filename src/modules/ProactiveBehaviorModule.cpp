...
ProactiveBehaviorModule::ProactiveBehaviorModule(ReminderRepository *repository,MemoryRepository *memories,QObject *parent)
    :FeatureModule(parent),m_repository(repository),m_memories(memories)
{
    ...
    m_memories->removeTimeBoundMemories();m_repository->cancelReminders(QStringLiteral("proactive_story_followup"));m_repository->archiveExpiredCognitiveRecords(QDateTime::currentDateTime());
...
{
    if(r.status==QStringLiteral("planned")&&r.scheduledAt.isValid()&&r.explicitRequest)
        return m_repository->addCognitiveReminderAtomic(r,QStringLiteral("cognitive_")+r.recordType,r.scheduledAt,payload);
    return m_repository->addCognitiveRecord(r);
}
...
bool ProactiveBehaviorModule::handleUserMessage(const QString &message,QString *reply)
{
    ...
    // 只有纯时间补充才允许完成上一条待确认提醒。若本轮本身包含一条
    // 新的完整提醒，必须先按新意图处理，不能把它的时间偷给旧提醒。
    const CognitiveRouteResult currentRoute=CognitiveRoutingRole::route(text,now);
    const auto awaiting=m_repository->loadCognitiveRecords({QStringLiteral("awaiting_confirmation")});
    if(currentRoute.action.isEmpty()&&!awaiting.isEmpty()){bool d=false,c=false;QDateTime at=CognitiveRoutingRole::parseTime(text,now,&d,&c);if(at.isValid()&&c){
    ...
    const CognitiveRouteResult route=currentRoute;
