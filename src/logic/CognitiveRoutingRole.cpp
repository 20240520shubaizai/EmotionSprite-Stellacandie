...
QDateTime CognitiveRoutingRole::parseTime(const QString &text,const QDateTime &now,bool *hasDate,bool *hasClock)
{
    ...
    QRegularExpression clockRe(QStringLiteral("(上午|中午|下午|晚上|今晚|凌晨)?\\s*(\\d{1,2})(?:点|时|:|：)(?:(\\d{1,2})分?)?"));auto cm=clockRe.match(normalized);
    if(cm.hasMatch()){int hour=cm.captured(2).toInt(),minute=cm.captured(3).isEmpty()?0:cm.captured(3).toInt();const QString period=cm.captured(1);if((period==QStringLiteral("下午")||period==QStringLiteral("晚上")||period==QStringLiteral("今晚"))&&hour<12)hour+=12;if(period==QStringLiteral("中午")&&hour<11)hour+=12;if(period==QStringLiteral("凌晨")&&hour==12)hour=0;time=QTime(hour,minute);clockFound=time.isValid();}
    QRegularExpression minutesRe(QStringLiteral("(\\d{1,4})\\s*分钟后"));auto mm=minutesRe.match(text);if(mm.hasMatch()){if(hasDate)*hasDate=true;if(hasClock)*hasClock=true;return now.addSecs(mm.captured(1).toInt()*60);}
    QRegularExpression hoursRe(QStringLiteral("(\\d{1,3})\\s*小时后"));auto hm=hoursRe.match(text);if(hm.hasMatch()){if(hasDate)*hasDate=true;if(hasClock)*hasClock=true;return now.addSecs(hm.captured(1).toInt()*3600);}
...
QString CognitiveRoutingRole::normalizedSubject(const QString &message)
{
    ...
    s.remove(QRegularExpression(QStringLiteral("(?:今天|今晚|明天|后天|大后天|\\d{1,3}天后|\\d{1,4}分钟后|\\d{1,3}小时后|\\d{1,2}月\\d{1,2}(?:日|号)?|(?:上午|中午|下午|晚上|凌晨)?\\s*\\d{1,2}(?:点|时|:|：)(?:\\d{1,2}分?)?)")));
    s.remove(QRegularExpression(QStringLiteral("^(?:我|要|去|准备|计划|一定要|必须|务必)+")));
